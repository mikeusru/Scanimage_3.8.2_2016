...
try
    ...
    if strcmp(state.spc.init.dllname, 'TH260lib')
        RY_imaging = 1; %~state.spc.acq.spc_takeFLIM; %save etc
        RY_imaging2 = 1; %COUNTER;
        RY_imaging3 = 1; %Acquisition through TH260lib
    else
...
RY_imagingSave = RY_imaging || RY_imaging3;
...
try
    ...
    if dia.acq.doRibbonTransform %Misha
        currenttempImage = makeRibbonStripe(frameFinalData);
        discardLineAfterReshape = false;
        discardLastLine = false;
        startLine=1;
        stopLine=linesPerStripe;
        stopLineLoopDiscard = stopLine;
    ...
    if RY_imaging
        for channelCounter = 1:state.init.maximumNumberOfInputChannels
            if state.acq.acquiringChannel(channelCounter) % if statemetnt only gets executed when there is a channel to acquire.
                if ~RY_imaging3
                    if dia.acq.doRibbonTransform %misha
                        state.acq.acquiredData{1}{channelCounter} = currenttempImage{channelCounter};
                    else
